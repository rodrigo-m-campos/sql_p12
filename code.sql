...
Join ExchangeRate ER ON E.CurrencyId = ER.CurrencyFrom 
					 ...
					 AND ER.ExchangeDate <= E.ExpenseDate  -- Use the most recent rate before or on the expense date
...
---3.6
select AU.FirstName, AU.LastName, AG.GroupName,COUNT(N.NotificationId where N.IsRead = 'N') AS UnreadNotifications
From AppUser AU
Join Notification N ON AU.AppUserId = N.RecipientId
Join AppGroup AG ON N.AppGroupId = AG.AppGroupId
